...
% will track all particles in the IMAGES specifies in SETTINGSFILE (either 
...
for i=1:size(ims,2)
  ...
  for th=length(thr):-1:1
    ...
    if th==length(thr)
      clear blobs
      blobs=blobrec(ims(i).cdata,thr(th),properties);
    else    
      blobs=blobrec(ims(i).cdata,thr(th),properties,blobs,maxdist,edgedist);
    end 
