function [pimg,pos2D,T,Tinv,aRoi]=cali2Dg(fname,bgnd,lnoise,Size_dot,Delta_dot,aRoi)
%
...
for icam = 1:ncams
    ...
    if ~isempty(bgnd)
        ...
        bkg=imopen(im,strel('disk',ceil(Size_dot/2)));
    end
    
    Iimg=imsubtract(im,bkg);
    Iimg=im+(bkg-imb)*4;
    ...
    [IT,xx,yy ] = imtransform(im, Tinv, 'Xdata' , [-122 90], 'Ydata', [-55  55], 'XYScale',0.08);
